...
def enhance_text_with_headings(summarized_text, model_choice, temperature, openai_api_key, language="Italian"):
    ...
    template = ChatPromptTemplate.from_messages([
        ("system", f"""
        Sei un assistente AI specializzato nel migliorare testi riassunti e aggiungere titoletti appropriati in {language}. Il tuo compito è rivedere il testo fornito, migliorarlo, inserire titoletti che riflettano il contenuto di ciascuna sezione ed espandendo ogni sezione.
...
def pdf_summary_s():
    ...
    if uploaded_file is not None:
        ...
        if 'chunks' in st.session_state:
            ...
            if st.button("Riassumi e Genera"):
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    try:
                        ...
                        # Mostra il riassunto del primo blocco generato
                        if i == 0:
                ...
                # Memorizza il testo migliorato per permettere il download senza resettare la pagina
                st.session_state['enhanced_text'] = enhanced_text
                
                # Genera immediatamente il file audio e memorizza il risultato
                audio_path = text_to_speech_edge(enhanced_text, language, speaker)
                with open(audio_path, "rb") as audio_file:
                    st.session_state['audio_bytes'] = BytesIO(audio_file.read())

                # Memorizza i file txt e docx
                st.session_state['txt_data'] = create_txt(enhanced_text)
                st.session_state['docx_data'] = create_docx(enhanced_text)
                
                st.write("### Testo migliorato con titoletti:")
                st.write(enhanced_text)

        # Permetti il download del file ZIP solo se tutti i dati sono stati generati
        if all(key in st.session_state for key in ['txt_data', 'docx_data', 'audio_bytes']):
            zip_data = create_zip_file(st.session_state['txt_data'], st.session_state['docx_data'], st.session_state['audio_bytes'], pdf_filename)
