...
def main():
    ...
    menu = ["Home", "Elaborazione PDF", "Query", "Gestisci Indici", "Aggiungi PDF a Indice"]
    pagina = st.radio("Navigazione", menu, horizontal=True)

    # Messaggio di benvenuto nella barra laterale
    st.sidebar.title("Benvenuto in Edubot")
    st.sidebar.write("""
    ...
    - **Gestisci Indici**: Visualizza e cancella gli indici esistenti.
    - **Aggiungi PDF a Indice**: Aggiungi nuovi documenti PDF a un indice esistente.
    """)
    ...
    **Disclaimer:**
    ...
    elif pagina == "Aggiungi PDF a Indice":
        st.header("Aggiungi PDF a Indice")
