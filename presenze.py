...
def inserisci_lezione_studente():
    ...
    if st.button("Registra Presenza"):
        ...
        st.session_state.presenze_db = pd.concat([st.session_state.presenze_db, new_row], ignore_index=True)
        save_data()
        st.success("Lezione registrata con successo.")
...
def main():
    ...
    if 'logged_in_user' not in st.session_state:
        ...
        if user_role == "Insegnante":
            menu = st.sidebar.selectbox("Menu", ["Visualizza Lezioni", "Visualizza Tutti gli Studenti", "Visualizza Database Completo", "Inserisci Presenze", "Rimuovi Presenze", "Logout"])
