...
available_models = {
    ...
    "phi3": "phi3",
    "llama3": "llama3"  # Modello aggiunto
}

# Inizializza le variabili di sessione se non già fatto
if 'selected_model' not in st.session_state:
    st.session_state.selected_model = list(available_models.keys())[0]  # Default al primo modello
...
# Instanzia il modello selezionato
def instantiate_model(selected_model, temperature):
    try:
        if selected_model in ["phi3", "llama3"]:
            # Configurazione specifica per modelli che non richiedono chiavi API
            return ChatOpenAI(
            ...
            # Configurazione per modelli che richiedono una chiave API
            return ChatOpenAI(
                temperature=temperature,
                model_name=available_models[selected_model],
                api_key=api_key
        ...
        st.error(f"Errore durante l'istanza del modello: {str(e)}")
        return None
...
def query_page():
    ...
    def list_subfolders(db_path):
        """Ritorna una lista delle sotto-cartelle nella cartella specificata."""
        try:
    ...
    # Selezione del modello (ora sempre visibile)
    ...
    # Campo di input per la query (ora appare dopo la selezione del modello)
    ...
    if st.button("Invia"):
        # Instanzia il modello selezionato quando si invia la query
        model = instantiate_model(st.session_state.selected_model, temperature)

        if model is None:
            st.error("Errore durante l'istanza del modello selezionato.")
            return
        ...
        def format_documents(all_documents):
            ...
            for doc in all_documents:
                ...
                # Gestione del caso in cui la pagina non sia un numero
                try:
        ...
        # Esegui la query e visualizza la risposta
        ...
        # Aggiorna la variabile della conversazione
        st.session_state.conversazione += (
            "-----------------------------\n"
    ...
    # Visualizza la domanda e la risposta corrente
    ...
    # Interruttore per mostrare/nascondere lo storico delle conversazioni
    mostra_storico = st.checkbox("Mostra storico delle conversazioni", value=False)

    # Mostra lo storico delle conversazioni se l'interruttore è selezionato
    if mostra_storico:
    ...
    # Bottone per scaricare la conversazione
    st.download_button(
